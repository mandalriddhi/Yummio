...
restaurant = input("Enter the restaurant address: ")
...
restaurant_full = f"{restaurant}, {city}, {state}, {country}"

# Geocode source, destination, and restaurant
source_result = gmaps.geocode(source_full)
destination_result = gmaps.geocode(destination_full)
restaurant_result = gmaps.geocode(restaurant_full)
if not source_result or not destination_result or not restaurant_result:
...
restaurant_location = restaurant_result[0]['geometry']['location']

# Display restaurant coordinates
restaurant_coords = (restaurant_location['lat'], restaurant_location['lng'])
print(f"\nRestaurant coordinates:")
print(f"Latitude: {restaurant_coords[0]}")
print(f"Longitude: {restaurant_coords[1]}")
...
print(f"\nFound {len(directions_result)} possible routes from source to destination.")
...
# Add restaurant marker with icon and coordinates in popup
folium.Marker(
    restaurant_coords,
    popup=folium.Popup(
        f"<b>Restaurant</b><br>"
        f"Latitude: {restaurant_coords[0]}<br>"
        f"Longitude: {restaurant_coords[1]}<br>"
        f"Distance from route: {distance:.2f}m", 
        max_width=300
    ),
    tooltip='Restaurant',
...
if on_route:
    print(f"Restaurant is on the way. It is approximately {distance:.2f} meters from the preferred route.")
    ...
    print(f"This restaurant is not on the preferred route. It is {distance:.2f} meters away from the nearest point.")
...
# Add restaurant analysis to Excel with clear coordinates
